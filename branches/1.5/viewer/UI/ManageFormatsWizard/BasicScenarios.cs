...
namespace LogJoint.UI
{
	public class OperationOverExistingFormatScenario : IFormatsWizardScenario
	{
		...
		public bool Next()
		{
			...
			{
				if (choosePage.GetSelectedFormat() is RegularGrammar.UserDefinedFormatFactory)
				{
	...
	public class DeleteFormatScenario: IFormatsWizardScenario
	{
		...
		public bool Next()
		{
			try
			{
				System.IO.File.Delete(format.FileName);
			}
			catch (Exception e)
			{
	...
	public class ModifyRegexBasedFormatScenario : IFormatsWizardScenario
	{
		public ModifyRegexBasedFormatScenario(IWizardScenarioHost host)
		{
			...
			identityPage = new FormatIdentityPage(false);
		...
		{
			string fname = System.IO.Path.GetFileName(factory.FileName);
		...
		{
			formatDoc.Load(factory.FileName);
			ResetFormatDocument();
