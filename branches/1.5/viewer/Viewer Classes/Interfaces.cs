...
namespace LogJoint
{
	...
	public interface ILogMedia: IDisposable
	{
		...
		void Update();
		/// <summary>
		/// Returns false when the media becomes inavailable. This may happen
		/// when, for example, the log file gets deleted. Property's value 
		/// gets updated only after call to Update().
		/// </summary>
		bool IsAvailable { get; }
		/// <summary>
		/// Reference to Stream object that represents media's content.
		/// After call to Update() this property may start returning 
		/// another reference. Stream's content may change
