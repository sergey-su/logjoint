...
namespace LogJoint.Log4net
{
	public class RollingFilesMedia : ILogMedia
	{
		...
		readonly Source trace = Source.EmptyTracer;
		...
		readonly string baseFileDirectory;
		...
		public RollingFilesMedia(LogMedia.IFileSystem fileSystem, IConnectionParams connectParams, MediaInitParams p)
		{
			trace = p.Trace;
			using (trace.NewFrame)
			{
				...
				baseFileName = connectParams["path"];
				trace.Info("Base file name: {0}", baseFileName);
				...
				try
				{
					...
					this.baseFileNameFirstChar = GetFileNameFirstChar(baseFileName);
					this.baseFileDirectory = Path.GetDirectoryName(baseFileName);
					trace.Info("Base file name first character: {0}", baseFileNameFirstChar);
					trace.Info("Base file directory: {0}", baseFileDirectory);

					this.concatStream = new ConcatReadingStream();
					this.tempThreads = new LogSourceThreads(Source.EmptyTracer, new Threads(), null);

					this.fsWatcher = fileSystem.CreateWatcher();
					this.fsWatcher.Path = Path.GetDirectoryName(baseFileName);
		...
		void InitialSearchForFiles()
		{
			using (trace.NewFrame)
			{
				foreach (string fname in fileSystem.GetFiles(baseFileDirectory, baseFileNameFirstChar + "*"))
				{
		...
		void fsWatcher_Created(object sender, FileSystemEventArgs e)
		{
			using (trace.NewFrame)
			{
				...
				if (IsFileARolledLog(e.FullPath))
				{
					...
					try
					{
						...
						if (simpleMedia == null)
						{
							...
							simpleMedia = new SimpleFileMedia(
								owner.fileSystem,
								SimpleFileMedia.CreateConnectionParamsFromFileName(owner.baseFileDirectory + "\\" + FileName),
						...
						if (firstMessageTime == null)
						{
							...
							using (IPositionedMessagesProvider provider = (IPositionedMessagesProvider)Activator.CreateInstance(
									owner.initParams.ProviderType, owner.tempThreads, SimpleMedia, owner.initParams.FormatInfo))
							{
								owner.trace.Info("Provider created");

								provider.UpdateAvailableBounds(false);
								owner.trace.Info("Bounds found");

								MessageBase first = PositionedMessagesUtils.ReadNearestMessage(provider, provider.BeginPosition);
								if (first == null)
								{
