...
#pragma warning disable 0252
    ...
    internal sealed class RegexCharClass {
        ...
        ***************************************************************************/
        ...
        internal static bool IsECMAWordChar(char ch, CharInClassOptimize opt) {
            return CharInClass(ch, ECMAWordClass, opt);
        }
        ...
        internal static bool IsWordChar(char ch, CharInClassOptimize opt) {
            return CharInClass(ch, WordClass, opt);
        }
        ...
        internal static bool CharInClass(char ch, String set, CharInClassOptimize opt) {
            return opt.CharInClassOptimized(ch, set);
        }

        internal unsafe class CachedCharInClass
        {
            public const char CacheSize = (char)256;
            public String set = "";
            public bool* data = (bool*)IntPtr.Zero;

            ~CachedCharInClass()
            {
                IntPtr dataPtr = new IntPtr(data);
                if (dataPtr != IntPtr.Zero)
                    System.Runtime.InteropServices.Marshal.FreeCoTaskMem(dataPtr);
            }
        };

        internal class CharInClassOptimize
        {
            public CachedCharInClass last = new CachedCharInClass();
            public Dictionary<string, CachedCharInClass> fullCache = new Dictionary<string, CachedCharInClass>();

            public unsafe bool CharInClassOptimized(char ch, String set)
            {
                if (ch < CachedCharInClass.CacheSize)
                {
                    if ((object)last.set == set)
                    {
                        return last.data[ch];
                    }
                    if (!fullCache.TryGetValue(set, out last))
                    {
                        var cache = new CachedCharInClass();
                        cache.set = set;
                        cache.data = (bool*)System.Runtime.InteropServices.Marshal.AllocCoTaskMem(CachedCharInClass.CacheSize * sizeof(bool));
                        for (char i = (char)0; i < CachedCharInClass.CacheSize; ++i)
                            cache.data[i] = CharInClassRecursive(i, set, 0);
                        last = cache;
                        fullCache.Add(set, cache);
                    }
                    return last.data[ch];
                }
