...
namespace System.Text.RegularExpressions.LogJointVersion {
    ...
    public class Match : Group {
        ...
        static void CopyIntArray(ref int[] dest, int[] src)
        {
            if (src == null)
            {
                dest = null;
        ...
        internal void CopyFromAndTidy(Match src, int textpos)
        {
            // Copy the members that are needed for Tidy()
            src._matchcount.CopyTo(this._matchcount, 0);
            src._matches.CopyTo(this._matches, 0);
            for (int i = 0; i < this._matches.Length; ++i)
                CopyIntArray(ref this._matches[i], src._matches[i]);
            this._balancing = src._balancing;
            ...
            if (this._capcoll != null)
                this._capcoll._capturesAreValid = false;

            // Match members
            this._regex = src._regex;
            this._textbeg = src._textbeg;
            this._textpos = src._textpos;
            this._textend = src._textend;
            this._textstart = src._textstart;
            if (_groupcoll != null)
                _groupcoll._groupsAreValid = false;
        }
        ...
        internal virtual void Reset(Regex regex, String text, int textbeg, int textend, int textstart)
        {
            ...
            if (_groupcoll != null)
                _groupcoll._groupsAreValid = false;
            if (_capcoll != null)
                _capcoll._capturesAreValid = false;
        }
        ...
        static public Match Synchronized(Match inner) {
            ...
            for (int i = 0; i < numgroups; i++) {
                ...
                System.Text.RegularExpressions.LogJointVersion.Group.Synchronized(group);
            }
