...
namespace System.Text.RegularExpressions.LogJointVersion {
    ...
    abstract public class RegexRunner {
        ...
        internal RegexCharClass.CharInClassOptimize charInClassOptimize = new RegexCharClass.CharInClassOptimize();
         ...
         */
        protected internal Match Scan(Regex regex, String text, int textbeg, int textend, int textstart, int prevlen, bool quick, Match resultMatch) {
            int bump;
            int stoppos;
            bool initted = false;
            bool timeboxed;
            ...
            for (int iteration=0;;++iteration) {
                ...
                if (FindFirstChar()) {
                    ...
                    if (runmatch._matchcount[0] > 0) {
                        return TidyMatch(quick, resultMatch);
                    }
        ...
        private void InitMatch() {
            ...
            if (runmatch == null) {
                runmatch = runregex.CreateNewMatch(runtext, runtextbeg, runtextend - runtextbeg, runtextstart);
            }
            else {
         ...
         */
        private Match TidyMatch(bool quick, Match resultMatch) {
            if (!quick) {
                if (resultMatch != null) {
                    // Fill up provided Match and return it

                    resultMatch.CopyFromAndTidy(runmatch, runtextpos);
                ...
                if (resultMatch != null) {
                    resultMatch.Reset(runregex, runtext, runtextbeg, runtextend, runtextstart);
                    resultMatch.Tidy(runtextpos);
                }
        ...
        protected bool IsBoundary(int index, int startpos, int endpos) {
            return (index > startpos && RegexCharClass.IsWordChar(runtext[index - 1], charInClassOptimize)) !=
                   (index < endpos && RegexCharClass.IsWordChar(runtext[index], charInClassOptimize));
        }

        protected bool IsECMABoundary(int index, int startpos, int endpos) {
            return (index > startpos && RegexCharClass.IsECMAWordChar(runtext[index - 1], charInClassOptimize)) !=
                   (index < endpos && RegexCharClass.IsECMAWordChar(runtext[index], charInClassOptimize));
        }
