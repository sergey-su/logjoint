...
namespace LogJoint.Postprocessing.Timeline
{
    public class TimelineBuilder
    {
        ...
        {
            var visitor = new Visitor(this, postprocessorOutput, isLastEventsSet);
        ...
        void AddEvent(Event sourceEvt, EventType type, ITimelinePostprocessorOutput currentPostprocessorOutput, ITimeOffsets currentTimeOffsets)
        {
        ...
        void HandleActivityEvent(ActivityEventBase evt, ActivityType type,
            ITimelinePostprocessorOutput currentPostprocessorOutput, ITimeOffsets currentTimeOffsets, string? activityMatchingId = null)
        {
            ...
            if (evt.Type == ActivityEventType.Begin || evt.Type == ActivityEventType.PotentialBegin)
            {
                ...
                AddPhases(activity, evt, currentPostprocessorOutput);
            }
            else if (evt.Type == ActivityEventType.End)
            {
                ...
                {
                    AddPhases(startedActivity, evt, currentPostprocessorOutput);
        ...
        void AddPhases(StartedActivity startedActivity, ActivityEventBase evt, ITimelinePostprocessorOutput currentPostprocessorOutput)
        {
        ...
        {
            required internal EventInfo begin;
            required internal ITimelinePostprocessorOutput beginOwner;
            internal string? activityMatchingId;
            required internal List<ActivityMilestoneInfo> milestones;
            required internal List<ActivityPhaseInfo> phases;
        ...
        class Visitor : IEventsVisitor
        {
            ...
            public Visitor(TimelineBuilder builder, ITimelinePostprocessorOutput postprocessorOutput, bool isLastEventsSet)
            {
            ...
            {
                builder.HandleActivityEvent(evt, ActivityType.Procedure, currentPostprocessorOutput, currentTimeOffsets);
            }

            void IEventsVisitor.Visit(ObjectLifetimeEvent evt)
            {
                builder.HandleActivityEvent(evt, ActivityType.Lifespan, currentPostprocessorOutput, currentTimeOffsets);
            }

            void IEventsVisitor.Visit(NetworkMessageEvent evt)
            {
                builder.HandleActivityEvent(evt, ToNetworkActivityType(evt.Direction), currentPostprocessorOutput, currentTimeOffsets, evt.ActivityId);
            }
            ...
            {
                builder.AddEvent(evt, EventType.UserAction, currentPostprocessorOutput, currentTimeOffsets);
            }

            void IEventsVisitor.Visit(APICallEvent evt)
            {
                builder.AddEvent(evt, EventType.APICall, currentPostprocessorOutput, currentTimeOffsets);
            }

            void IEventsVisitor.Visit(EndOfTimelineEvent evt)
            {
                var eventInfo = new EventInfo(evt, currentTimeOffsets);
                builder.SetOrigin(eventInfo);
                if (currentIsLastEventsSet)
                    builder.endOfTimelineEventInfo = eventInfo;
            }
        }
    };
}
