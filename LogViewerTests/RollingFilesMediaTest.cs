...
namespace LogJointTests
{
	...
	public class RollingFilesMediaTest
	{
		...
		class MessagesProvider : IPositionedMessagesProvider
		{
			ILogMedia media;
			public const int EmptyFileContent = 0;
			public const int InvalidFileContent = 666;

			public MessagesProvider(LogSourceThreads threads, ILogMedia media, object fmtInfo)
			{
				this.media = media;
			}

			#region IPositionedMessagesProvider Members
			...
			public class Parser : IPositionedMessagesParser
			{
				...
				public MessageBase ReadNext()
				{
					if (messageRead)
						return null;
					messageRead = true;
					return new Content(0, null, time, "", Content.SeverityFlag.Info);
				}
				public void Dispose()
				{
				}
		...
		RollingFilesMedia CreateMedia(FileSystemImpl fs, string baseFile)
		{
			StreamBasedMediaInitParams p = new StreamBasedMediaInitParams(
				Source.EmptyTracer,
				typeof(MessagesProvider),
				new StreamBasedFormatInfo(typeof(RollingFilesMedia)));
		...
		public void InvaidFileInTheGroupMustBeIgnored()
		{
			...
			fs.AddFile("a0.log", 1);
			fs.AddFile("a1.log", 2);
			fs.AddFile("a2.log", MessagesProvider.InvalidFileContent);
			fs.AddFile("a3.log", 4);
			using (RollingFilesMedia media = CreateMedia(fs, @"\a0.log"))
			{
				media.Update();
		...
		{
			FileSystemImpl fs = new FileSystemImpl();
			fs.AddFile("a0.log", MessagesProvider.EmptyFileContent);
			fs.AddFile("a1.log", 2);
			fs.AddFile("a2.log", MessagesProvider.EmptyFileContent);
			fs.AddFile("a3.log", 4);
			using (RollingFilesMedia media = CreateMedia(fs, @"\a0.log"))
			{
				media.Update();
